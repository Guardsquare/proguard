...
/**
 * Test class for {@link KotlinAliasNameObfuscator#visitKotlinDeclarationContainerMetadata(Clazz, KotlinDeclarationContainerMetadata)}.
 * Tests the visitKotlinDeclarationContainerMetadata method which resets the name factory
 * and delegates to typeAliasesAccept.
 */
public class KotlinAliasNameObfuscatorClaude_visitKotlinDeclarationContainerMetadataTest {
     ...
     * This verifies that the name factory is reset before processing type aliases.
     */
    @Test
    ...
    public void testVisitKotlinDeclarationContainerMetadata_passesObfuscatorAsVisitor() {
        ...
        verify(mockMetadata, times(1)).typeAliasesAccept(mockClazz, obfuscator);
    }

    /**
     * Tests that visitKotlinDeclarationContainerMetadata calls reset before typeAliasesAccept.
     * This verifies the correct order of operations.
     */
    @Test
    public void testVisitKotlinDeclarationContainerMetadata_callsResetBeforeTypeAliasesAccept() {
        // Arrange
        ...
        // Assert - verify reset is called before typeAliasesAccept
        inOrder.verify(mockNameFactory).reset();
        inOrder.verify(mockMetadata).typeAliasesAccept(mockClazz, obfuscator);
    }
    ...
    public void testVisitKotlinDeclarationContainerMetadata_canBeCalledMultipleTimes() {
        ...
        // Assert - verify that each call triggers reset and typeAliasesAccept
    ...
    public void testVisitKotlinDeclarationContainerMetadata_withDifferentClazz_passesCorrectClazz() {
        ...
        verify(mockMetadata, times(1)).typeAliasesAccept(mockClazz, obfuscator);
        verify(mockMetadata, times(1)).typeAliasesAccept(mockClazz2, obfuscator);
    }
    ...
    public void testVisitKotlinDeclarationContainerMetadata_withNullClazz_delegatesToMetadata() {
        ...
        // Verify that reset was called and typeAliasesAccept was made with null clazz
        verify(mockNameFactory, times(1)).reset();
     ...
     * The clazz is only passed to typeAliasesAccept, not used directly.
     */
    @Test
    public void testVisitKotlinDeclarationContainerMetadata_doesNotInteractWithClazz() {
        ...
        // Assert - verify no direct interactions with clazz (it's only passed to typeAliasesAccept)
        verifyNoInteractions(mockClazz);
    }

    /**
     * Tests that visitKotlinDeclarationContainerMetadata delegates all work to typeAliasesAccept.
     * Verifies that only typeAliasesAccept is called on the metadata, not other methods.
     */
    @Test
    public void testVisitKotlinDeclarationContainerMetadata_onlyCallsTypeAliasesAccept() {
        // Act
        obfuscator.visitKotlinDeclarationContainerMetadata(mockClazz, mockMetadata);

        // Assert - verify only typeAliasesAccept is called, not propertiesAccept, functionsAccept, etc.
        verify(mockMetadata, times(1)).typeAliasesAccept(mockClazz, obfuscator);
        verifyNoMoreInteractions(mockMetadata);
    }

    /**
     * Tests the complete workflow: reset then typeAliasesAccept.
    ...
    public void testVisitKotlinDeclarationContainerMetadata_passesCorrectParameters() {
        ...
        verify(specificMetadata, times(1)).typeAliasesAccept(specificClazz, obfuscator);
    }

    /**
     * Tests that visitKotlinDeclarationContainerMetadata with sequential calls
     * resets the factory before each typeAliasesAccept call.
     */
    @Test
    public void testVisitKotlinDeclarationContainerMetadata_sequentialCalls_resetEachTime() {
        ...
        // Assert - reset should be called before each typeAliasesAccept
        verify(mockNameFactory, times(3)).reset();
        verify(metadata1, times(1)).typeAliasesAccept(mockClazz, obfuscator);
        verify(metadata2, times(1)).typeAliasesAccept(mockClazz, obfuscator);
        verify(metadata3, times(1)).typeAliasesAccept(mockClazz, obfuscator);
    }
}
