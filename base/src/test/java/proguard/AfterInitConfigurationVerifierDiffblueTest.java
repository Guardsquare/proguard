...
class AfterInitConfigurationVerifierDiffblueTest {
   ...
   * AfterInitConfigurationVerifier#AfterInitConfigurationVerifier(Configuration)}
   */
  @Test
  ...
  void testNewAfterInitConfigurationVerifier() throws MalformedURLException {
    ...
    // Act and Assert
    assertEquals(
        "proguard.AfterInitConfigurationVerifier",
        new AfterInitConfigurationVerifier(configuration).getName());
  }

  /**
   * Test {@link AfterInitConfigurationVerifier#execute(AppView)}.
   *
   * <ul>
   ...
   * </ul>
   *
   * <p>Method under test: {@link AfterInitConfigurationVerifier#execute(AppView)}
   */
  @Test
  @DisplayName(
  ...
  @Tag("ContributionFromDiffblue")
  @ManagedByDiffblue
  @MethodsUnderTest({"void AfterInitConfigurationVerifier.execute(AppView)"})
  void testExecute_whenClassPoolClassesAcceptDoesNothing_thenCallsClassesAccept()
    ...
    configuration.whyAreYouKeeping = new ArrayList<>();
    configuration.zipAlign = 1;
    AfterInitConfigurationVerifier afterInitConfigurationVerifier =
        new AfterInitConfigurationVerifier(configuration);

    ClassPool programClassPool = mock(ClassPool.class);
    doNothing().when(programClassPool).classesAccept(Mockito.<ClassVisitor>any());

    // Act
    afterInitConfigurationVerifier.execute(
