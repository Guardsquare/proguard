...
/**
 * Test class for {@link LibraryKeepChecker#checkClassSpecifications(List)}.
 * Tests the checkClassSpecifications method with various List configurations.
 */
public class LibraryKeepCheckerClaude_checkClassSpecificationsTest {
    ...
    /**
     * Tests checkClassSpecifications with specification having null className.
     * Verifies that specifications with null className are skipped gracefully.
     */
    @Test
    public void testCheckClassSpecifications_withNullClassName() {
        ...
        List<KeepClassSpecification> specifications = new ArrayList<>();
        ClassSpecification classSpec = new ClassSpecification();
        classSpec.className = null; // Null className should be skipped
        KeepClassSpecification keepSpec = new KeepClassSpecification(
                true, false, false, false, false, false, false, false,
                null, classSpec
        );
        ...
        // Act & Assert - should not throw any exception, null className is skipped
        assertDoesNotThrow(() -> checker.checkClassSpecifications(specifications));
    }

    /**
     * Tests checkClassSpecifications with specification having wildcard className.
     * Verifies that specifications with wildcard patterns work correctly.
     */
    @Test
    public void testCheckClassSpecifications_withWildcardClassName() {
        ...
        List<KeepClassSpecification> specifications = new ArrayList<>();
        ClassSpecification classSpec = new ClassSpecification();
        classSpec.className = "com/example/*";
        KeepClassSpecification keepSpec = new KeepClassSpecification(
                true, false, false, false, false, false, false, false,
                null, classSpec
        );
    ...
    /**
     * Tests checkClassSpecifications with specification having specific className.
     * Verifies that specifications with specific class names work correctly.
     */
    @Test
    public void testCheckClassSpecifications_withSpecificClassName() {
        // Arrange
        ClassPool programClassPool = new ClassPool();
        ClassPool libraryClassPool = new ClassPool();
        WarningPrinter warningPrinter = mock(WarningPrinter.class);
        ...
        List<KeepClassSpecification> specifications = new ArrayList<>();
        ClassSpecification classSpec = new ClassSpecification();
        classSpec.className = "com/example/MyClass";
        KeepClassSpecification keepSpec = new KeepClassSpecification(
                true, false, false, false, false, false, false, false,
                null, classSpec
        );
    ...
    /**
     * Tests checkClassSpecifications with multiple specifications having different class names.
     * Verifies that specifications with various class names are processed correctly.
     */
    @Test
    public void testCheckClassSpecifications_withDifferentClassNames() {
        // Arrange
        ClassPool programClassPool = new ClassPool();
        ClassPool libraryClassPool = new ClassPool();
        WarningPrinter warningPrinter = mock(WarningPrinter.class);
        ...
        // Add specification with specific class name
        ClassSpecification classSpec1 = new ClassSpecification();
        classSpec1.className = "com/example/ClassA";
        specifications.add(new KeepClassSpecification(
                true, false, false, false, false, false, false, false,
                null, classSpec1
        ));

        // Add specification with wildcard
        ClassSpecification classSpec2 = new ClassSpecification();
        classSpec2.className = "com/example/*";
        specifications.add(new KeepClassSpecification(
                true, false, false, false, false, false, false, false,
                null, classSpec2
        ));

        // Add specification with null className
        ClassSpecification classSpec3 = new ClassSpecification();
        classSpec3.className = null;
        specifications.add(new KeepClassSpecification(
                true, false, false, false, false, false, false, false,
                null, classSpec3
        ));
    ...
    /**
     * Tests checkClassSpecifications with specification having markConditionally.
     * Verifies that specifications with markConditionally=true work correctly.
     */
    @Test
    public void testCheckClassSpecifications_withMarkConditionally() {
        // Arrange
        ClassPool programClassPool = new ClassPool();
        ClassPool libraryClassPool = new ClassPool();
        WarningPrinter warningPrinter = mock(WarningPrinter.class);
    ...
    /**
     * Tests checkClassSpecifications with null warning printer.
     * Verifies behavior when warning printer is null and might be used.
     */
    @Test
    public void testCheckClassSpecifications_withNullWarningPrinter() {
        // Arrange
        ClassPool programClassPool = new ClassPool();
        ClassPool libraryClassPool = new ClassPool();
        LibraryKeepChecker checker = new LibraryKeepChecker(
        ...
        // Act & Assert - should not throw if warning printer is not used
        assertDoesNotThrow(() -> checker.checkClassSpecifications(specifications));
    }
}
