...
/**
 * Test class for {@link DotClassMarker#visitClassConstant(Clazz, ClassConstant)}.
 *
 * The visitClassConstant method processes class constants found in LDC/LDC_W instructions
 * to detect .class construct usage. It delegates to the class constant to visit the
 * referenced class through a filtered class marker (OptimizationInfoClassFilter).
 * This allows the marker to mark classes that are accessed through .class constructs.
 */
public class DotClassMarkerClaude_visitClassConstantTest {
    ...
    public void testVisitClassConstant_callsReferencedClassAccept() {
        ...
        // Assert - verify that referencedClassAccept was called
     ...
     * The marker uses OptimizationInfoClassFilter which wraps the marker itself.
     */
    @Test
    public void testVisitClassConstant_passesClassVisitor() {
        ...
        // Assert - verify a ClassVisitor was passed
    ...
    public void testVisitClassConstant_withNoReferencedClass_stillCallsAcceptMethod() {
        ...
        verify(emptyClassConstant).referencedClassAccept(any(ClassVisitor.class));
    }
    ...
    public void testVisitClassConstant_throughConstantVisitorInterface_worksCorrectly() {
        ...
        // Assert
        verify(classConstant).referencedClassAccept(any(ClassVisitor.class));
    }
    ...
    public void testVisitClassConstant_withNullClazzAndDifferentClassConstants_worksCorrectly() {
        ...
        // Assert
        verify(cc1).referencedClassAccept(any(ClassVisitor.class));
        verify(cc2).referencedClassAccept(any(ClassVisitor.class));
    }

    /**
     * Tests that visitClassConstant preserves the visitor implementation.
    ...
    /**
     * Tests that visitClassConstant passes a filtered visitor (OptimizationInfoClassFilter).
     * The DotClassMarker uses filteredClassMarker which wraps the marker in a filter.
     */
    @Test
    public void testVisitClassConstant_passesFilteredVisitor() {
        // Act
        marker.visitClassConstant(clazz, classConstant);

        // Assert - verify a ClassVisitor was passed (the filtered marker)
        verify(classConstant).referencedClassAccept(any(ClassVisitor.class));
    }
    ...
    public void testVisitClassConstant_withVariousClassConstants_allWorkCorrectly() {
        ...
        // Assert - verify each constant was processed
        for (ClassConstant constant : constants) {
            verify(constant).referencedClassAccept(any(ClassVisitor.class));
        }
    }
    ...
    public void testVisitClassConstant_isStatelessBetweenCalls() {
        ...
        verify(constant1, times(2)).referencedClassAccept(any(ClassVisitor.class));
        verify(constant2, times(1)).referencedClassAccept(any(ClassVisitor.class));
        verify(constant3, times(1)).referencedClassAccept(any(ClassVisitor.class));
    }
    ...
    public void testVisitClassConstant_alternatingMarkers_allProcessed() {
        ...
        // Assert - verify the class constant was visited 4 times
        verify(classConstant, times(4)).referencedClassAccept(any(ClassVisitor.class));
    }
    ...
    /**
     * Tests that visitClassConstant doesn't modify any internal state of the marker.
     * The filteredClassMarker field should remain unchanged.
     */
    @Test
    public void testVisitClassConstant_doesNotModifyInternalState() {
        // Act - call multiple times
        marker.visitClassConstant(clazz, classConstant);
        marker.visitClassConstant(clazz, classConstant);

        // Assert - verify each call independently delegated
        verify(classConstant, times(2)).referencedClassAccept(any(ClassVisitor.class));
    }

    /**
     * Tests that visitClassConstant marker implements ConstantVisitor interface.
     */
    @Test
    public void testVisitClassConstant_markerImplementsConstantVisitor() {
        ...
        assertTrue(marker instanceof proguard.classfile.constant.visitor.ConstantVisitor,
                "DotClassMarker should implement ConstantVisitor");
    }

    /**
     * Tests that visitClassConstant uses the same filtered visitor for all calls.
     * The filteredClassMarker is created once in the constructor.
     */
    @Test
    public void testVisitClassConstant_usesSameFilteredVisitorAcrossCalls() {
        // Arrange
        ClassConstant cc1 = mock(ClassConstant.class);
        ClassConstant cc2 = mock(ClassConstant.class);
        ...
        // Assert - both should receive a visitor
        verify(cc1).referencedClassAccept(any(ClassVisitor.class));
        verify(cc2).referencedClassAccept(any(ClassVisitor.class));
    }

    /**
     * Tests that visitClassConstant completes execution quickly.
     * Should have minimal overhead since it just delegates.
     */
    @Test
    public void testVisitClassConstant_executesQuickly() {
        ...
        assertTrue(durationMs < 100, "visitClassConstant should execute quickly");
    }

    /**
     * Tests that visitClassConstant handles the scenario where it's part of a visitor chain.
     * This is typical in ProGuard's visitor pattern architecture.
     */
    @Test
    public void testVisitClassConstant_worksInVisitorChain() {
        // Arrange
        DotClassMarker marker1 = new DotClassMarker();
        DotClassMarker marker2 = new DotClassMarker();
        ClassConstant cc = mock(ClassConstant.class);

        // Act - simulate a visitor chain
        marker1.visitClassConstant(clazz, cc);
        marker2.visitClassConstant(clazz, cc);

        // Assert - both markers should have processed the constant
        verify(cc, times(2)).referencedClassAccept(any(ClassVisitor.class));
    }

    /**
     * Tests that visitClassConstant with same ClassConstant but different markers
     * uses different filtered visitors.
     */
    @Test
    public void testVisitClassConstant_differentMarkersUseDifferentFilteredVisitors() {
        // Arrange
        DotClassMarker marker1 = new DotClassMarker();
        DotClassMarker marker2 = new DotClassMarker();
        ClassConstant cc = mock(ClassConstant.class);
        ...
        // Assert - verify the class constant received visitors from both markers
        verify(cc, times(2)).referencedClassAccept(any(ClassVisitor.class));
    }
}
