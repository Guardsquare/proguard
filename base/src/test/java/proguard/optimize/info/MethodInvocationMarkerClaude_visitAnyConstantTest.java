...
/**
 * Test class for {@link MethodInvocationMarker#visitAnyConstant(Clazz, Constant)}.
 *
 * The visitAnyConstant method is an empty implementation (no-op) that serves as a default
 * handler in the ConstantVisitor pattern. The MethodInvocationMarker only processes
 * specific constant types (StringConstant and AnyMethodrefConstant) through specialized methods.
 * This method provides the default no-op behavior for all other constant types.
 */
public class MethodInvocationMarkerClaude_visitAnyConstantTest {
    ...
    public void testVisitAnyConstant_implementsConstantVisitorInterface() {
        // Assert - MethodInvocationMarker should be a ConstantVisitor
        assertTrue(marker instanceof proguard.classfile.constant.visitor.ConstantVisitor,
                "MethodInvocationMarker should implement ConstantVisitor");
    }

    /**
     * Tests that visitAnyConstant doesn't affect the filteredMethodMarker field.
     ...
     * Class and Utf8 constants should be handled without issues.
     */
    @Test
    ...
    public void testVisitAnyConstant_servesAsDefaultForUnspecializedConstants() {
        ...
        // Act - these constants don't have specialized methods in MethodInvocationMarker
        marker.visitAnyConstant(clazz, intConst);
        marker.visitAnyConstant(clazz, longConst);
        marker.visitAnyConstant(clazz, floatConst);
        marker.visitAnyConstant(clazz, doubleConst);
     ...
     * Field references don't trigger method invocation counting.
     */
    @Test
     ...
     * that are not method references.
     */
    @Test
    public void testVisitAnyConstant_withNonMethodRefConstants_doesNotThrow() {
        // Arrange
        Constant fieldrefConstant = mock(FieldrefConstant.class);
