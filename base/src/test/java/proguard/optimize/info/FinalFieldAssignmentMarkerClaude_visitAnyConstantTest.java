...
/**
 * Test class for {@link FinalFieldAssignmentMarker#visitAnyConstant(Clazz, Constant)}.
 *
 * The visitAnyConstant method is an empty implementation (no-op) that serves as a default
 * handler in the ConstantVisitor pattern. The FinalFieldAssignmentMarker only processes
 * FieldrefConstant through the specialized visitFieldrefConstant method. This method
 * provides the default no-op behavior for all other constant types.
 */
public class FinalFieldAssignmentMarkerClaude_visitAnyConstantTest {
     ...
     * The referencedMethod field should remain unchanged by this no-op method.
     */
    @Test
    ...
    public void testVisitAnyConstant_withNewlyCreatedMarker_worksCorrectly() {
        ...
        FinalFieldAssignmentMarker newMarker = new FinalFieldAssignmentMarker();
    ...
    /**
     * Tests that visitAnyConstant works with MethodrefConstant, which should not be processed.
     * Only FieldrefConstant has special handling via visitFieldrefConstant.
     */
    @Test
    public void testVisitAnyConstant_withMethodrefConstant_doesNotThrow() {
        // Arrange
        MethodrefConstant methodrefConstant = mock(MethodrefConstant.class);

        // Act & Assert
        assertDoesNotThrow(() -> marker.visitAnyConstant(clazz, methodrefConstant));
    ...
    /**
     * Tests that visitAnyConstant works with InterfaceMethodrefConstant.
     */
    @Test
    public void testVisitAnyConstant_withInterfaceMethodrefConstant_doesNotThrow() {
        // Arrange
        InterfaceMethodrefConstant interfaceMethodrefConstant = mock(InterfaceMethodrefConstant.class);

        // Act & Assert
        assertDoesNotThrow(() -> marker.visitAnyConstant(clazz, interfaceMethodrefConstant));
    ...
    /**
     * Tests that visitAnyConstant works with NameAndTypeConstant.
     */
    @Test
    public void testVisitAnyConstant_withNameAndTypeConstant_doesNotThrow() {
        // Arrange
        NameAndTypeConstant nameAndTypeConstant = mock(NameAndTypeConstant.class);

        // Act & Assert
        assertDoesNotThrow(() -> marker.visitAnyConstant(clazz, nameAndTypeConstant));
    ...
    /**
     * Tests that visitAnyConstant is truly a default handler that does nothing
     * for constants that are not FieldrefConstant (which has specialized handling).
     */
    @Test
    public void testVisitAnyConstant_defaultHandlerForNonFieldrefConstants() {
        // Arrange - various constant types that should all be no-ops
        ...
        // Act
        marker.visitAnyConstant(clazz, stringConstant);
        marker.visitAnyConstant(clazz, classConstant);
        marker.visitAnyConstant(clazz, methodrefConstant);
        marker.visitAnyConstant(clazz, integerConstant);
    ...
    /**
     * Tests that visitAnyConstant can alternate with other operations without issues.
     */
    @Test
    public void testVisitAnyConstant_alternatingWithOtherCalls_noIssues() {
        // Arrange
    ...
    /**
     * Tests that visitAnyConstant handles being called on the same marker instance repeatedly.
     */
    @Test
    public void testVisitAnyConstant_sameMarkerReusedMultipleTimes_consistentBehavior() {
        // Arrange
        Constant[] constants = {
        ...
        // Act - use same marker for all calls
        for (Constant c : constants) {
            marker.visitAnyConstant(clazz, c);
        }

        // Assert - no interactions with any constant
        verifyNoInteractions(clazz);
        for (Constant c : constants) {
