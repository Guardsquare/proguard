...
class KotlinMetadataVerifierDiffblueTest {
   ...
   *       LibraryClass#LibraryClass(int, String, String, KotlinMetadata)}.
  ...
  @Test
  @DisplayName(
      "Test execute(AppView); when ClassPool(Clazz[]) with classes is LibraryClass(int, String, String, KotlinMetadata); then calls classesAccept(ClassVisitor)")
  @Tag("ContributionFromDiffblue")
  @ManagedByDiffblue
  @MethodsUnderTest({"void KotlinMetadataVerifier.execute(AppView)"})
  void testExecute_whenClassPoolWithClassesIsLibraryClass_thenCallsClassesAccept() {
    // Arrange
    KotlinMetadataVerifier kotlinMetadataVerifier =
        new KotlinMetadataVerifier(mock(Configuration.class));
