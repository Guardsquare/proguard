...
class FilterBuilderDiffblueTest {
  ...
  @Test
  @DisplayName(
      "Test buildFilter(); given array of JCheckBox with JCheckBox(String, boolean) with 'foo' and 'true'")
  @Tag("ContributionFromDiffblue")
  @ManagedByDiffblue
  @MethodsUnderTest({"java.lang.String FilterBuilder.buildFilter()"})
  void testBuildFilter_givenArrayOfJCheckBoxWithJCheckBoxWithFooAndTrue() {
    // Arrange
    JCheckBox[] checkBoxes = new JCheckBox[] {new JCheckBox("foo", true)};
    FilterBuilder filterBuilder = new FilterBuilder(checkBoxes, 'A');
